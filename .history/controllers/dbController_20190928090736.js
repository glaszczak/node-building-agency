...
async function addNewContractor() {
    ...
    try {
        let newConctractor = { fullName: req.body.fullName, city: req.body.city, address: req.body.address }
        let sql = `INSERT INTO tbl_contractors SET ?`
        console.log(newConctractor)
        const results = await client.query(sql, newConctractor, (err, result) => {
            if (err) throw err
            result.redirect('/contractors')
        })
        return results.rows
    }
    catch (e) {
